...
final class NilaiPesertaController extends Controller
{
    ...
    {
        return view('admin.nilai_peserta.index');
    }

    /**
     * Menampilkan rekap nilai peserta yang sudah terdaftar pada kelas.
     * Admin dapat melihat seluruh kelas dan seluruh mentor.
     */
    public function list(Request $request): JsonResponse
    {
        ...
        $rekapNilaiPendaftaran = DB::table('progres_kuis')
            ...
            ->select(
                ...
                DB::raw(
                    'MAX(progres_kuis.diserahkan_pada) AS terakhir_mengerjakan'
                )
            );
        ...
        return $this->transactionService->handleWithDataTable(
            function () use (
                ...
                $data = DB::table('pendaftaran')
                    ...
                    ->when(
                        $request->filled('id_mentor'),
                        fn ($query) => $query->where(
                            'kelas.id_pemilik',
                            $request->input('id_mentor')
                        )
                    )
                    ->when(
                    ...
                    ->select(
                        ...
                        'kelas.id_pemilik as id_mentor',

                        DB::raw("COALESCE(mentor.nama, '-') AS mentor_nama"),

                        DB::raw(
                            'COALESCE(
                                rekap_kuis_kelas.total_kuis,
                                0
                            ) AS total_kuis'
                        ),
                ...
                if ($request->filled('status_nilai')) {
                    ...
                    $data = $data
                        ->filter(
                            fn ($row) =>
                            $this->statusNilaiKey($row) === $statusNilai
                        )
                        ->values();
                }
            ...
            [
                ...
                'status_nilai' => fn ($row) =>
                    $this->statusNilaiBadge($row),
            ]
        );
    }

    /**
     * Menampilkan detail seluruh kuis berdasarkan satu pendaftaran.
     */
            ...
            function () use ($id) {
                ...
                $nilaiKuis = DB::table('kuis')
                    ...
                    ->select(
                        ...
                        DB::raw(
                            'COALESCE(
                                rekap_histori.jumlah_percobaan,
                                0
                            ) AS jumlah_percobaan'
                        )
                    )
                    ...
                    ->map(function ($row) {
                        $sudahMengerjakan =
                            ! is_null($row->diserahkan_pada);

                        $jumlahPercobaan =
                            (int) $row->jumlah_percobaan;
    ...
    /**
     * Menghapus pendaftaran peserta beserta data progres yang terkait.
     */
    public function delete(string $id): JsonResponse
    {
        ...
        if (! $pendaftaran) {
            return $this->responseService->errorResponse(
                'Data pendaftaran peserta tidak ditemukan.',
                404
            );
        }

        return $this->transactionService->handleWithTransaction(
            function () use ($pendaftaran) {
                $this->pendaftaranService->delete($pendaftaran);

                return $this->responseService->successResponse(
                    'Pendaftaran peserta beserta nilai dan progresnya berhasil dihapus.'
                );
            }
        );
    }
