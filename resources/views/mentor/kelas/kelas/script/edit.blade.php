<script defer>
    ...
    $('#form_edit').on('show.bs.modal', function(e) {
        ...
        const detail = '{{ route('mentor.kelas.kelas.show', [':id']) }}';
        $('#edit_id_kategori').on('change', function() {
            ...
            if (id_kategori) {
                fetchDataDropdown("{{ route('mentor.api.kelas.kategori_sub', [':id']) }}".replace(':id',
        ...
        DataManager.fetchData(detail.replace(':id', id))
            .then(function(response) {
                if (response.success) {
                    ...
                    fetchDataDropdown("{{ route('mentor.api.kelas.kategori') }}", '#edit_id_kategori',
                        'kategori', 'nama', () => {
                            $("#edit_id_kategori").val(data.id_kategori).trigger("change");
                            fetchDataDropdown("{{ route('mentor.api.kelas.kategori_sub', [':id']) }}"
        ...
        $('#bt_submit_edit').off('submit').on('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    const update = '{{ route('mentor.kelas.kelas.update', [':id']) }}';
