<script defer>
    $('#form_edit').on('show.bs.modal', function(e) {
        ...
        fetchDataDropdown("{{ route('mentor.api.kelas.mentor') }}", '#edit_id_mentor', 'mentor', 'nama');

        DataManager.fetchData(detail.replace(':id', id))
            .then(function(response) {
                if (response.success) {
                    const data = response.data;
                    $('#edit_peran').val(data.peran);
                    $('#edit_id_mentor').val(data.id_mentor).trigger('change');
        ...
        $('#bt_submit_edit').off('submit').on('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    formData.append('id_mentor', $('#edit_id_mentor').val());
                    formData.append('peran', $('#edit_peran').val());

                    const update = '{{ route('mentor.kelas.mentor.update', [':id']) }}';
