<script defer>
    $('#form_create').on('show.bs.modal', function() {
        fetchDataDropdown("{{ route('mentor.api.kelas.mentor') }}", '#create_id_mentor', 'mentor', 'nama');
        ...
        $('#bt_submit_create').off('submit').on('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    formData.append('id_mentor', $('#create_id_mentor').val());
                    formData.append('peran', $('#create_peran').val());

                    const action = "{{ route('mentor.kelas.mentor.store') }}";
