...
(function (e) {
    ...
    e.fn.dataTableExt.oApi.fnColReorder = function (a, b, c, f, h) {
        var g, l, k = a.aoColumns.length;
        var p = function (w, x, E) {
            if (w[x] && "function" !== typeof w[x]) {
                var B = w[x].split("."), C = B.shift();
                isNaN(1 * C) || (w[x] = E[1 * C] + "." + B.join("."))
            }
        };
        if (b != c) if (0 > b || b >= k) this.oApi._fnLog(a, 1, "ColReorder 'from' index is out of bounds: " + b); else if (0 > c || c >= k) this.oApi._fnLog(a, 1, "ColReorder 'to' index is out of bounds: " +
            c); else {
            var m = [];
            var d = 0;
            for (g = k; d < g; d++) m[d] = d;
            ...
            if (null !== a.aaSortingFixed) for (d = 0, g = a.aaSortingFixed.length; d < g; d++) a.aaSortingFixed[d][0] = q[a.aaSortingFixed[d][0]];
            d = 0;
            for (g = k; d < g; d++) {
                var n = a.aoColumns[d];
                m = 0;
                for (l = n.aDataSort.length; m < l; m++) n.aDataSort[m] = q[n.aDataSort[m]];
                n.idx = q[n.idx]
            }
            ...
            if (a.aoColumns[b].bVisible) {
                ...
                l = null;
                for (d = c < b ? c : c + 1; null === l && d < k;) l = this.oApi._fnColumnIndexToVisible(a, d), d++;
                m = a.nTHead.getElementsByTagName("tr");
                d = 0;
                for (g = m.length; d < g; d++) A(m[d], p, l);
                if (null !== a.nTFoot) for (m = a.nTFoot.getElementsByTagName("tr"), d = 0, g = m.length; d < g; d++) A(m[d], p, l);
                d = 0;
            ...
            for (g = a.aoData.length; d < g; d++) {
                l = a.aoData[d];
                if (n = l.anCells) for (v(n, b, c), m = 0, p = n.length; m < p; m++) n[m] && n[m]._DT_CellIndex && (n[m]._DT_CellIndex.column = m);
                "dom" !== l.src && Array.isArray(l._aData) && v(l._aData, b, c)
            }
            d = 0;
            for (g = a.aoHeader.length; d < g; d++) v(a.aoHeader[d], b, c);
            if (null !== a.aoFooter) for (d = 0, g = a.aoFooter.length; d < g; d++) v(a.aoFooter[d], b, c);
    ...
    e.extend(r.prototype, {
        ...
        fnOrder: function (a, b) {
            ...
            b || (b = "toCurrent");
            var c = this.fnOrder(), f = this.s.dt.aoColumns;
            ...
            var a = this, b = this.s.dt.aoColumns.length, c = this.s.dt.nTable, f;
            ...
            if (h) if (a.s.dt._bInitComplete) b = y(h), a._fnOrderColumns.call(a, b); else {
                var g = !1;
        ...
        _fnOrderColumns: function (a) {
            ...
            if (a.length != this.s.dt.aoColumns.length) this.s.dt.oInstance.oApi._fnLog(this.s.dt, 1, "ColReorder - array reorder does not match known number of columns. Skipping."); else {
                ...
                b && (e.fn.dataTable.Api(this.s.dt).rows().invalidate(), "" === this.s.dt.oScroll.sX && "" === this.s.dt.oScroll.sY || this.s.dt.oInstance.fnAdjustColumnSizing(!1),
            ...
            var c = this, f = e(a.target).closest("th, td").offset();
            b =
                parseInt(e(b).attr("data-column-index"), 10);
            ...
            }).on("mouseup.ColReorder touchend.ColReorder",
                function (h) {
            ...
            for (var f = function (d) {
                ...
                for (var d = 0; d < b.s.aoTargets.length - 1; d++) if (b.s.aoTargets[d].x !== b.s.aoTargets[d + 1].x) return b.s.aoTargets[d]
            }, g = function () {
                for (var d = b.s.aoTargets.length - 1; 0 < d; d--) if (b.s.aoTargets[d].x !== b.s.aoTargets[d - 1].x) return b.s.aoTargets[d]
            ...
            var a = this.s.dt.aoColumns, b = this._fnIsLtr();
            this.s.aoTargets.splice(0, this.s.aoTargets.length);
            var c = e(this.s.dt.nTable).offset().left, f = [];
            e.each(a, function (l, k) {
                if (k.bVisible && "none" !== k.nTh.style.display) {
                    k = e(k.nTh);
                    var p = k.offset().left;
                    b && (p += k.outerWidth());
                    f.push({index: l, bound: p});
                    c = p
                } else f.push({index: l, bound: c})
            });
            var h = f[0];
            ...
            var a = "" !== this.s.dt.oScroll.sX || "" !== this.s.dt.oScroll.sY,
                b = this.s.dt.aoColumns[this.s.mouse.targetIndex].nTh, c = b.parentNode, f = c.parentNode,
                h = f.parentNode, g = e(b).clone();
